	...
	**/
	...
	@:keep public function toString() {
		return "Controller"
			...
			+ allAccesses.length
			+ ", "
			+ (pad == null || pad.index < 0 ? '<NoPad>' : '"' + pad.name + '"#' + pad.index)
			+ ")";
	}
...
class InputBinding<T:EnumValue> {
	...
	public function toString() {
		var all = [];
		if (isLStick && padPos == null)
			all.push("LSTICK" + (isX ? "_X" : "_Y") + (invert ? "-" : "+"));
		if (isRStick && padPos == null)
			all.push("RSTICK");
		if (padNeg != null)
			all.push(Std.string(padNeg));
		if (padPos != null)
			all.push(Std.string(padPos));
		if (padButton != null)
			all.push(Std.string(padButton));
		if (kbNeg >= 0)
			all.push(Key.getKeyName(kbNeg));
		if (kbPos >= 0 && kbNeg != kbPos)
			all.push(Key.getKeyName(kbPos));
		return all.join("/");
	}
	...
	public inline function isDown(pad:hxd.Pad) {
		return !isLStick
			&& pad.isDown(input.getPadButtonId(padButton))
			|| Key.isDown(kbPos)
			|| Key.isDown(kbNeg)
			|| pad.isDown(input.getPadButtonId(padPos))
			|| pad.isDown(input.getPadButtonId(padNeg))
			|| isLStick
			&& dn.M.fabs(getValue(pad)) > analogDownDeadZone;
	}
